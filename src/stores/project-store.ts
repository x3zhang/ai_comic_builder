...
import { writePersistedStoryboardVersion } from "@/lib/storyboard-version-persist";
...
interface Project {
  ...
  /** Which storyboard version the loaded `shots` belong to (episode fetch / ?versionId=). */
...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  fetchProject: async (id: string, episodeId?: string, versionId?: string) => {
    ...
    if (
      ...
      data &&
      typeof data === "object" &&
      "storyboardVersionId" in data
    ) {
      writePersistedStoryboardVersion(
        id,
        episodeId,
        (data as { storyboardVersionId: string | null }).storyboardVersionId,
      );
    }
