...
/**
 * fluent-ffmpeg does not ship ffmpeg/ffprobe. Prefer env overrides, then
 * @ffmpeg-installer/* so `video_assemble` works on Windows without a global PATH.
 */
function ensureFfmpegPaths(): void {
  ...
  if (envFfmpeg) {
    if (fs.existsSync(envFfmpeg)) {
      ffmpeg.setFfmpegPath(envFfmpeg);
    } else {
      console.warn(`[FFmpeg] FFMPEG_PATH is set but file not found: ${envFfmpeg}`);
    }
  } else {
    try {
      ...
      if (mod?.path && fs.existsSync(mod.path)) {
        ffmpeg.setFfmpegPath(mod.path);
      }
    } catch (e) {
      console.warn(
        "[FFmpeg] Bundled ffmpeg unavailable (optional platform package). Install ffmpeg on PATH or set FFMPEG_PATH.",
        e,
      );
    }
  }

  const envProbe = process.env.FFPROBE_PATH?.trim();
  if (envProbe) {
    if (fs.existsSync(envProbe)) {
      ffmpeg.setFfprobePath(envProbe);
    } else {
      console.warn(`[FFmpeg] FFPROBE_PATH is set but file not found: ${envProbe}`);
    }
  } else {
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      ...
      if (mod?.path && fs.existsSync(mod.path)) {
        ffmpeg.setFfprobePath(mod.path);
      }
    } catch {
      /* ffprobe optional for concat; some flows still work */
    }
  }
}
