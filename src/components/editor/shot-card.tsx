...
export function ShotCard({
  ...
  function refImageToAssetPatch(r: RefImage) {
    ...
    // Never send `fileUrl: null` when we only have a pending placeholder in memory.
    // Debounced prompt saves close over stale `allRefItems`; after `single_frame_generate`
    // completes, a late PUT would otherwise clear freshly written URLs in the DB.
    if (r.imagePath) patch.fileUrl = r.imagePath;
    return patch;
  }
