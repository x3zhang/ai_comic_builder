...
export default function ProjectLayout({
  ...
  // `/project/.../episodes/[episodeId]/...` loads the store from the episode API in
  // `episodes/[episodeId]/layout.tsx`. A project-level GET here races with that
  // fetch and overwrites episode-scoped shots, versions, and assets — causing a
  // brief wrong state (e.g. two asset "versions") until the episode response wins.
  const isEpisodeScopedRoute = useMemo(() => {
    const parts = pathname?.split("/").filter(Boolean) ?? [];
    const i = parts.indexOf("episodes");
    return i >= 0 && Boolean(parts[i + 1]);
  }, [pathname]);

  useEffect(() => {
    if (isEpisodeScopedRoute) return;
    void fetchProject(id);
  }, [id, isEpisodeScopedRoute, fetchProject]);

  if (!isEpisodeScopedRoute && (loading || !project)) {
    return (
