...
export default function EpisodePreviewPage() {
  ...
  useEffect(() => {
    if (versionIdFromUrl && params?.id && params.episodeId) {
      void fetchProject(params.id, params.episodeId, versionIdFromUrl);
    }
  }, [versionIdFromUrl, params?.id, params?.episodeId, fetchProject]);
  ...
  async function handleAssemble() {
    ...
    const effectiveVersionId =
      versionIdFromUrl ?? project.storyboardVersionId ?? undefined;
    try {
      const res = await apiFetch(`/api/projects/${project.id}/generate`, {
        ...
        body: JSON.stringify({
          ...
          payload: effectiveVersionId ? { versionId: effectiveVersionId } : {},
          episodeId,
        }),
      });
      await res.json();
    ...
    setAssembling(false);
    if (episodeId) {
      await fetchProject(project.id, episodeId, effectiveVersionId);
    }
  }
