...
/** Refresh episode-scoped project data without dropping ?versionId= (store holds current `storyboardVersionId`). */
async function refetchCurrentEpisodeProject() {
  const { fetchProject, project: p, currentEpisodeId: ep } = useProjectStore.getState();
  if (!p?.id || !ep) return;
  await fetchProject(p.id, ep, p.storyboardVersionId ?? undefined);
}
...
export default function EpisodeStoryboardPage() {
  ...
  // Derived: keep explicit user tab selection; else match API-loaded version
  // (sessionStorage / ?versionId=); else latest. Avoids a first-paint flash where
  // `_selectedVersionId` is null and we would wrongly highlight `versions[0]`
  // while `shots` still belong to `storyboardVersionId`.
  const selectedVersionId = (_selectedVersionId && versions.some((v) => v.id === _selectedVersionId))
    ? _selectedVersionId
    : (project?.storyboardVersionId && versions.some((v) => v.id === project.storyboardVersionId))
      ? project.storyboardVersionId
      : (versions[0]?.id ?? null);
  ...
  async function handleBatchGenerateFrames(overwrite = false) {
    ...
    await refetchCurrentEpisodeProject();
    ...
    <div className="animate-page-in space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {totalShots > 0 && (
            <Link
              href={`/${locale}/project/${project!.id}/episodes/${useProjectStore.getState().currentEpisodeId}/preview${(selectedVersionId || project?.storyboardVersionId) ? `?versionId=${selectedVersionId || project?.storyboardVersionId}` : ""}`}
      ...
      <div className="rounded-2xl border border-[--border-subtle] bg-white p-4 space-y-3">
        ...
        <CharactersInlinePanel
          ...
          onUpdate={() => void refetchCurrentEpisodeProject()}
        />
