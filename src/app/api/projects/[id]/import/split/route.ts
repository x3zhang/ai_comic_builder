...
import { createLanguageModel, parseJsonFromLlmText } from "@/lib/ai/ai-sdk";
  ...
  try {
    const chunkResults = await Promise.all(
      chunks.map(async (chunk, idx) => {
        ...
        try {
          return parseJsonFromLlmText(result.text) as SplitEpisode[];
        } catch {
          ...
          return parseJsonFromLlmText(retry.text) as SplitEpisode[];
        }
      })
    );
