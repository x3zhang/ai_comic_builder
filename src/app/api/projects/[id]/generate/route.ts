...
`;
  ...
  const chunkErrors: { chunk: number; message: string }[] = [];
  ...
  const chunkResults = await Promise.all(
    sceneChunks.map(async (chunk, idx) => {
      ...
      try {
        ...
        chunkErrors.push({ chunk: idx + 1, message });
  ...
  if (allShots.length === 0) {
    const hint = chunkErrors[0]?.message;
    return NextResponse.json(
      {
        error: hint
          ? `Failed to generate shots: ${hint}`
...
IMPORTANT: Keep the same scene, characters, and narrative intent. Only rephrase to avoid safety filter triggers. Match the language of the original text.`;
  ...
  try {
    ...
    const parsed = parseJsonFromLlmText(text) as {
      prompt: string;
...
async function handleBatchFrameGenerate(
  ...
  // DashScope image API enforces low QPS; unbounded Promise.allSettled triggers
  // Throttling.RateQuota (429). Limit concurrent shots for dashscope only.
  const imageProtocol = modelConfig.image.protocol;
  const batchConcurrency =
    imageProtocol === "dashscope" ? 2 : Math.min(8, Math.max(1, allShots.length));

  // ── Per-shot generation (chunked concurrency) ──
  const total = allShots.length;
  let doneCount = 0;
  console.log(
    `[BatchFrameGenerate] Starting generation: 0/${total} (concurrency=${batchConcurrency}, protocol=${imageProtocol})`,
  );

  async function runFrameJob(shot: (typeof allShots)[number]) {
  ...
  const settled: PromiseSettledResult<Awaited<ReturnType<typeof runFrameJob>>>[] = [];
  for (let i = 0; i < allShots.length; i += batchConcurrency) {
    const slice = allShots.slice(i, i + batchConcurrency);
    settled.push(...(await Promise.allSettled(slice.map((shot) => runFrameJob(shot)))));
  }
...
async function handleVideoAssembleSync(projectId: string, payload?: Record<string, unknown>, episodeId?: string) {
  ...
  if (videoPaths.length === 0) {
    ...
    const withKeyframeVideo = projectShots.filter((s) => {
      const v = projectShotsLegacy.get(s.id);
      return !!v?.videoUrl;
    }).length;
    const withRefVideo = projectShots.filter((s) => {
      const v = projectShotsLegacy.get(s.id);
      return !!v?.referenceVideoUrl;
    }).length;
    return NextResponse.json(
      {
        error: "No video clips to assemble",
        detail: isReference
          ? `当前为参考图模式：需要有「参考视频」的成片文件。本集共 ${shotCount} 个镜头，其中 ${withRefVideo} 个已有 reference_video。请先批量/逐镜生成参考视频后再合成。`
          : `当前为关键帧模式：需要有「插值视频」的成片文件。本集共 ${shotCount} 个镜头，其中 ${withKeyframeVideo} 个已有 keyframe_video。请先批量/逐镜生成视频后再合成。`,
...
async function handleBatchVideoPrompt(
  ...
  if (vpBoundAgent) {
    ...
    try {
      const vpParsed = parseJsonFromLlmText(agentResult.text) as Array<Record<string, unknown>>;
...
async function handleGenerateRefPrompts(
  ...
  if (refBoundAgent) {
    ...
    try {
      const refParsed = parseJsonFromLlmText(agentResult.text) as Array<Record<string, unknown>>;
...
`;
  ...
  for (let bi = 0; bi < batches.length; bi++) {
    ...
    try {
      ...
      try {
        parsed = parseJsonArrayFromLlmText(result) as typeof parsed;
      } catch (e) {
        const m = e instanceof Error ? e.message : String(e);
        throw new Error(`Batch ${bi + 1}: ${m}`);
      }
...
async function handleGenerateKeyframePrompts(
  ...
  if (kpBoundAgent) {
    ...
    try {
      const kpParsed = parseJsonFromLlmText(agentResult.text) as Array<Record<string, unknown>>;
    ...
    allShots.map(async (shot) => {
      try {
        ...
        try {
          ...
          const m = e instanceof Error ? e.message : String(e);
          throw new Error(`Shot ${shot.sequence}: ${m}`);
        }
